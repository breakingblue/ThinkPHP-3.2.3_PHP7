...
/**
 * 获取和设置配置参数 支持批量定义
 * @param null|string|array $name   配置变量
 * @param null|mixed $value 配置值
 ...
 * @param string|mixed $default 不存在的时候默认值
...
function I($name, $default = '', $filter = null, $datas = null) {
    ...
    } elseif (C('VAR_AUTO_STRING')) {   // 默认强制转换位字符串
        $type = 's';
    }
 ...
 * @param string|mixed $connection 数据库连接信息
 * @return mixed|\Think\Model
 */
 ...
 * @param string $name 资源地址
 * @param string $layer 控制器层次名
 * @param int $level 控制器层次
 * @return bool|mixed|\Think\Controller
 */
 ...
 * @param string $tag 标签名称（行为参数无需传入）
 * @param null|mixed $params 传入的参数
 * @return mixed|void
 */
 ...
 * 浏览器友好的变量输出
 * @param mixed $var 变量
 * @param bool $echo 是否输出 默认为True 如果为false则返回输出字符串
 * @param null|string $label 标签 默认为空
...
/**
 * 设置当前页面的布局
 * @param string|false $layout 布局名称 为false的时候标识关闭布局
 */
 ...
 * @param bool|string $suffix 伪静态后缀，默认为true标识获取配置值
...
/**
 * 缓存管理
 * @param mixed $name 缓存名称，如果为数组标识进行缓存设置
 * @param string|mixed $value 缓存值
 * @param null|mixed $options 缓存参数
 * @return mixed|string
 */
 ...
 * @param string|mixed $value session值
 * @return null
 */
 ...
 * @param string|mixed $value cookie值
 * @param null|mixed $option cookie参数
 * @return null|mixed
 */
...
//                return array_map('urldecode', json_decode(MAGIC_QUOTES_GPC ? stripslashes($value) : $value, true));
                return array_map('urldecode', json_decode(stripslashes($value), true));
            } else {
                return $value;
            }
        } else {
        ...
        if(is_null($value)) {
            ...
            // 设置cookie
            if(is_array($value)) {
                $value = 'think:'.json_encode(array_map('urlcode', $value));
            }
